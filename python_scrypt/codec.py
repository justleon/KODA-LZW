...
libc.code.argtypes = [POINTER(c_uint8), c_uint32, POINTER(POINTER(c_uint32)), POINTER(c_uint32)]
...
def code(input:bytes):
    ...
    ret = libc.code(c_in_buf, c_uint32(len(input)), c_out_buf, c_out_buf_len)
    if ret:
